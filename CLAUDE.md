...
ui/styles.py             design system (session 10 "restrained fintech"
                         tokens — the header comment IS the spec):
                         4/8px spacing scale, CARD_PAD 20, four flat
                         surfaces (BG #0B0F1A → CARD → CARD_ALT → HOVER),
                         ONE accent #6C7FF2 (gradients retired; GRAD_*/
                         GRADIENT are compat aliases), semantic colors
                         carry meaning only (green=gains red=losses
                         amber=warnings), one hairline border
                         (BORDER_SOFT rgba; BORDER = hex twin for
                         matplotlib), RADIUS 10 / RADIUS_SM 6, Inter with
                         Segoe UI fallback, label_font() = the 8pt
                         uppercase letter-spaced micro-label (size still
